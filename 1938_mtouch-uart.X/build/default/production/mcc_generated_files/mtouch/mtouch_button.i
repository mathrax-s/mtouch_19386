...
# 41 "mcc_generated_files/mtouch/../mtouch/mtouch.h" 2
...
enum mtouch_button_state
{
    ...
    const mtouch_button_t mtouch_button_init[5u] =
    {
...
static mtouch_button_t mtouch_button[5u];

static _Bool button_reburst_request;
...
static _Bool Button_Check_AKS_Block (mtouch_button_t* button);

static __attribute__((inline)) void Button_Reburst_Request(mtouch_button_t* button);
static __attribute__((inline)) void Button_Reburst_Clear(void);
...
static void Button_State_NotPressed(mtouch_button_t* button)
{
    ...
    {
        (button->counter)++;
        if ((button->counter) > (mtouch_button_statecounter_t)((mtouch_button_statecounter_t)8u))
        {
            button->counter = (mtouch_button_statecounter_t)0;
            MTOUCH_Button_Initialize(button->name);
            Button_Reburst_Request(button);
    ...
    if(button->aks_group != No_AKS_Group)
    {
        if(Button_Check_AKS_Block(button))
        {
            ...
            (((0xffff)) - ((mtouch_button_baselinecounter_t)1024u));
...
static void Button_State_Pressed(mtouch_button_t* button)
{
    ...
    if ((button->counter) >= ((mtouch_button_statecounter_t)500u))
    {
        MTOUCH_Button_Initialize(button->name);
...
static void Button_Tick_helper(mtouch_button_t* button)
{
    ...
    {
        (button->counter)++;
        if(button->counter >= ((mtouch_button_baselinecounter_t)64u))
        {
            button->counter = (mtouch_button_statecounter_t)0;
...
{
    return (mtouch_button_reading_t)(button->reading - (mtouch_button_reading_t)((button->baseline)>>((uint8_t)4u)));
}
mtouch_buttonmask_t MTOUCH_Button_Buttonmask_Get(void)
{
...
static _Bool Button_Check_AKS_Block(mtouch_button_t* button)
{
    ...
    for(check_button = &mtouch_button[0];check_button <= &mtouch_button[5u -1];check_button++)
    {
        ...
        else
        {
            if(check_button->aks_group == currentAKSGroup)
            {
                ...
                {
                        if(check_button->state == MTOUCH_BUTTON_STATE_pressed || check_button->state == MTOUCH_BUTTON_STATE_pressDebounce)
                            return 1;
                         else
                            {
...
static __attribute__((inline)) void Button_Reburst_Request(mtouch_button_t* button)
{
    button_reburst_request = 1;
}

static __attribute__((inline)) void Button_Reburst_Clear(void)
{
    button_reburst_request = 0;
}
...
# 786 "mcc_generated_files/mtouch/mtouch_button.c"
static void Button_DefaultCallback(enum mtouch_button_names button) { }
