    ...
    while (1) {
        ...
        if (MTOUCH_Service_Mainloop()) {
            ...
            if (MTOUCH_Slider_isPositionChanged(0)) {
                sliderPostion = MTOUCH_Slider_Position_Get(0);
                f_slider = sliderPostion;
            }
            if (!MTOUCH_Slider_isPressed(0)) {
                f_slider = 0;

            }

            ave_slider = (ave_slider * 0.8) + (f_slider * 0.2);

            map_slider = (ave_slider / 4096)*100;

            EUSART_Write(map_slider);
        }
